...
public class VehiculoServiceImpl implements VehiculoService {
    ...
    public List<VehiculoDTO> obtenerDisponibles() {
        return obtenerDisponibles(null);
    }

    @Override
    public List<VehiculoDTO> obtenerDisponibles(Double capacidadMinima) {
        return vehiculoRepository.findByEstado(EstadoVehiculo.DISPONIBLE)
                .stream()
                .filter(v -> capacidadMinima == null || v.getCapacidad() >= capacidadMinima)
                .map(this::toDTO)
                .collect(Collectors.toList());
    }
    ...
    @Override
    public VehiculoDTO obtenerPorMatricula(String matricula) {
        Vehiculo vehiculo = vehiculoRepository.findByMatricula(matricula)
                .orElseThrow(() -> new VehiculoNotFoundException("Vehículo no encontrado con matrícula: " + matricula));
        return toDTO(vehiculo);
    }
